-- Lets an admin suppress the synthesized "primary order" row (see
-- ensurePrimaryOrderRow in src/lib/sale-list.ts) for a specific customer whose
-- own order_number has no matching sale_list_entries row, without hiding it
-- for every other customer in the same situation. This is a per-customer
-- exception, not a derivable rule — e.g. DEX INTERNATIONAL CO. and
-- KI SEOK, SEO both have real sale_list_entries that don't match their own
-- order_number, yet only DEX's synthetic row should be hidden, so there's no
-- data pattern to key off of instead of an explicit flag.
alter table public.customers
  add column hide_primary_order boolean not null default false;

-- Extends get_portal_profile's customer selection (see
-- 20260826000000_portal_profile_sale_list.sql) so the scan page can also
-- honor this flag — it calls the same ensurePrimaryOrderRow() as the admin
-- panel, but reads the customer through this RPC rather than the table.
    ...
    'customer', (
      select row_to_json(c) from (
        select id, order_number, member_account_number, full_name, company_name, contract_number,
               ...
               dispenser_type, filter_installed, installed_date, assigned_technician, hide_primary_order
...
-- One-time exception: DEX's own order_number (SK001-0006) doesn't match
-- either of its two real sale_list_entries rows, but its synthetic row
-- should be hidden per an explicit business decision (unlike every other
-- customer in the same structural situation, e.g. KI SEOK, SEO).
update public.customers
set hide_primary_order = true
where order_number = 'SK001-0006';
