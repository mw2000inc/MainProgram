...
// technically became due). These rows are indistinguishable from a
// manually-scheduled job (same table, same fields) so they're editable
// through the existing Schedule page/dialog as-is.
//
...
export async function runGenerateFilterChangeJobs(): Promise<AutomationResult> {
  ...
  const errorCount = errors.length + cpErrors.length
  return {
    ok: errorCount === 0,
    message: `Checked ${customers?.length ?? 0} customers (+${created} created) and ${linkedEntries?.length ?? 0} CP-System-linked orders (+${cpCreated} created), ${errorCount} error(s).`,
    detail: {
