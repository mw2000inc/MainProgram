...
// rather than deducting immediately and unconditionally the way this job
// always has. Left in the registry, triggerable manually from Settings >
// Automations, purely for the older single-product-per-job path
// (schedule_jobs.product_id/quantity, still settable in the full Schedule
// form) — existing historical stock_movements rows it already created are
// untouched.
//
// For every "filter_change" schedule job that's been marked completed (by
// an admin/technician — this never fires off the scheduled date alone,
// since a job can be rescheduled or never actually happen) with a product +
// quantity set, and hasn't already been deducted, inserts a stock_movements
// row and marks the job deducted.
//
// Policy, unchanged from the original cron route:
// - Idempotency: inventory_deducted_at on the job is the guard — once set,
//   a job is never deducted again. Belt-and-suspenders: also skip if a
//   stock_movements row already references this job (covers the case
//   where a prior run inserted the movement but failed before marking the
//   job).
...
export async function runFilterChangeInventoryDeduction(): Promise<AutomationResult> {
  ...
  return {
    ok: errors.length === 0,
    message: `Checked ${jobs?.length ?? 0} jobs — deducted ${deducted}, already had a movement ${alreadyHadMovement}, ${errors.length} error(s).`,
    detail: { checked: jobs?.length ?? 0, deducted, alreadyHadMovement, errors },
  }
}
