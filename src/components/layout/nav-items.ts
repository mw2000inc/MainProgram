...
export interface NavItem {
  ...
  group: 1 | 2 | 3
  // A technician's scope is Daily Report + their own Schedule, nothing else
  // — every other module is admin-only in practice (RLS blocks the
  // underlying data too; see the technician_role migration). Defaults to
  // false/hidden when omitted. See isTechnicianAllowedPath below, the
  // single source of truth (app)/layout.tsx's redirect guard reads too, so
  // "what's in the nav" and "what's actually reachable" can't drift apart.
  technicianVisible?: boolean
}

export const NAV_ITEMS: NavItem[] = [
  { href: "/", label: "Daily Report", icon: ClipboardList, group: 1, technicianVisible: true },
  { href: "/schedule", label: "Schedule", icon: CalendarClock, group: 1, technicianVisible: true },
